...
// It authenticates via token/machine_id query params, sends auth.success, then
...
func fakeWSServer(t *testing.T, events []wsMessage) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		q := r.URL.Query()
		if q.Get("token") == "" || q.Get("machine_id") == "" {
...
func TestWSClient_ReconnectOnDisconnect(t *testing.T) {
	...
	host := strings.TrimPrefix(server.URL, "http://")
	ws := NewWSClient("ws://"+host, "reconnect-token", 1, WSClientConfig{MachineID: 9}, func(WSEvent) {}, nil, func() map[string]interface{} { return nil })
...
func TestWSClient_FallbackWhenNoServer(t *testing.T) {
	ws := NewWSClient("ws://127.0.0.1:19999", "fallback-token", 1, WSClientConfig{MachineID: 9}, func(WSEvent) {}, nil, func() map[string]interface{} { return nil })
...
func TestWSClient_UserDeltaEvent(t *testing.T) {
	...
	ws := NewWSClient("ws://"+host, "delta-token", 1, WSClientConfig{MachineID: 9}, func(event WSEvent) {
		mu.Lock()
		received = append(received, event)
		mu.Unlock()
	}, nil, func() map[string]interface{} { return nil })
